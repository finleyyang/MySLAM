...
#undef GET_VALUE
	...
	ORBExtractor::ORBExtractor(int _nfeatures,
		float _scaleFactor, int _nlevels, int _iniThFAST, int _minThFAST) :
		mi_Features(_nfeatures), mf_scaleFactor(_scaleFactor), mi_Levels(_nlevels),
		mi_iniThFAST(_iniThFAST), mi_minThFAST(_minThFAST)
	{
		mv_scaleFactor.resize(mi_Levels);
		mv_levelSigma2.resize(mi_Levels);
		...
		float
			nDesiredFeaturesPerScale = mi_Features * (1 - factor) / (1 - (float)pow((double)factor, (double)mi_Levels));
			...
			while (lit != lNodes.end())
			{
				...
				else
				{
					...
					if (n1.mv_Keys.size() > 0)
					{
						...
						{
							nToExpand++;
							vSizeAndPointerToNode.emplace_back(n1.mv_Keys.size(), &lNodes.front());
					...
					if (n2.mv_Keys.size() > 0)
					{
						...
						{
							nToExpand++;
							vSizeAndPointerToNode.emplace_back(n2.mv_Keys.size(), &lNodes.front());
							lNodes.front().lit = lNodes.begin();
						}
					}
					if (n3.mv_Keys.size() > 0)
					{
						...
						{
							nToExpand++;
							vSizeAndPointerToNode.emplace_back(n3.mv_Keys.size(), &lNodes.front());
							lNodes.front().lit = lNodes.begin();
						}
					}
					if (n4.mv_Keys.size() > 0)
					{
						...
						{
							nToExpand++;
							vSizeAndPointerToNode.emplace_back(n4.mv_Keys.size(), &lNodes.front());
							lNodes.front().lit = lNodes.begin();
						}
					}
					lit = lNodes.erase(lit);
		...
		cv::OutputArray _descriptors)
	{
		...
		for (int level = 0; level < mi_Levels; ++level)
		{
			...
			if (level != 0)
			{
				float scale = mv_scaleFactor[level];
				for (std::vector<cv::KeyPoint>::iterator keypoint = keypoints.begin(),
					     keypointEnd = keypoints.end(); keypoint != keypointEnd; ++keypoint)
					keypoint->pt *= scale;
			}
	...
	void ORBExtractor::ComputePyramid(cv::Mat image)
	{
		for (int level = 0; level < mi_Levels; ++level)
		{
			...
			if (level != 0)
			{
				resize(mv_imagePyramid[level - 1],
					mv_imagePyramid[level],
					sz, 0, 0,
					cv::INTER_LINEAR);

				copyMakeBorder(mv_imagePyramid[level],
					temp,
