...
namespace my_slam
{
	...
	ORBExtractor::ORBExtractor(int _nfeatures, float _scaleFactor, int _nlevels, int _iniThFAST, int _minThFAST)
		: mi_Features(_nfeatures), mf_scaleFactor(_scaleFactor), mi_Levels(_nlevels), mi_iniThFAST(_iniThFAST),
		  mi_minnThFAST(_minThFAST)
	{
		mv_scaleFactor.resize(mi_Levels);
		mv_levelSigma2.resize(mi_Levels);
		mv_scaleFactor[0] = 1.f;
		mv_levelSigma2[0] = 1.f;
		...
		float factor = 1.f / mf_scaleFactor;
		...
		//将pattern0中的点以cv::point的形式转到pattern中
